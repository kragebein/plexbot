...
case $cmd in 
	'all')
		compact() {	
			echo "Service status:"
			echo ""
			echo -n "Unifi Controller..";if ping -c 1 unifi.wack 1>/dev/null 2>/dev/null; then echo "online"
			echo -ne "\tUnifi Service: "; if nc -z unifi.wack 8443 1>/dev/null 2>/dev/null; then echo "online";else echo "offline";fi
		else echo "offline";fi
		echo ""
		#torrent.wack
		echo -n "Torrent VM..";if ping -c 1 torrent.wack 1>/dev/null 2>/dev/null; then echo "online";
		echo -ne "\trtorrent: ";if nc -z torrent.wack 3907; then echo "up";else echo "down";fi
	else echo "offline";fi;remote_collect torrent
		echo -e "\tNåværende Last:   ↓$curdn ↑$curup"
		echo -e "\tUkentlig trafikk: ↓$rx ↑$tx tot:$ttotal"
		echo -e "\tMånedlig trafikk: $mtotal"
		echo ""
		#cr.wack
		echo -n "Div VM.."; if ping -c 1 cr.wack 1>/dev/null 2>/dev/null; then echo "online"
		echo -ne "\tPlexPy:";if nc -z cr.wack 8181; then echo "up";else echo "down";fi
		echo -en "\tOmbi:";if nc -z cr.wack 5000; then echo "up";else echo "down";fi
		echo -ne "\tCouchPotato: ";if nc -z cr.wack 5050; then echo "up";else echo "down";fi
	else echo "offline";fi
	echo ""
	#services.wack
	echo -n "Services VM.."; if ping -c 1 services.wack 1>/dev/null 2>/dev/null; then echo "online"
	echo -ne "\tJackett: ";if nc -z services.wack 9117; then echo "up";else echo "down";fi
	echo -ne "\tDNS: ";if dig @services.wack google.com 1>/dev/null 2>/dev/null; then echo "up";else echo "down";fi
else echo "offline";fi
echo ""
#plex.wack
echo -n "David VM.."; if ping -c 1 plex.wack 1>/dev/null 2>/dev/null; then echo "online"
echo -ne "\tPlex Media Server: ";if nc -z plex.wack 32400; then echo "up"; else echo "down";fi
	else echo "offline";fi;remote_collect plex
		echo -e "\tNåværende Last:   ↓$curdn ↑$curup"
		echo -e "\tUkentlig trafikk: ↓$rx ↑$tx tot:$ttotal"
		echo -e "\tMånedlig trafikk: $mtotal"
		echo ""
		#shell.wack
		echo -n "Shell VM.."; if ping -c 1 shell.wack 1>/dev/null 2>/dev/null; then echo "online"
		echo -ne "\tConnectable: ";if nc -z shell.wack 22; then echo "yes"; else echo "no";fi
	else echo "offline";fi
	echo ""
	#lazywack.no
	echo -n "lazywack.no.."; if ping -c 1 lazy.wack 1>/dev/null 2>/dev/null; then echo "online"
	echo -ne "\tnginx 80: ";if nc -z lazy.wack 80; then echo "up"; else echo "down";fi
	echo -ne "\tnginx 443: ";if nc -z lazy.wack 443; then echo "up"; else echo "down";fi
	#		remote_collect lazy
else echo "offline";fi
echo ""
#goliath.wack
echo -n "Goliath.."; if ping -c 1 10.0.0.34 1>/dev/null 2>/dev/null; then echo "online" 
echo -ne "\tSickRage: "; if nc -z 10.0.0.34 8081; then echo "up"; else echo "down";fi
echo -ne "\tNFS: ";if nc -z 10.0.0.34 2049; then echo "up"; else echo "down";fi
echo -ne "\tCrypto: "; if [ -f /drive/drive/.file ]; then echo "mounted";else echo "!!! not mounted !!!"; fi
echo -ne "\tRar: "; if [ -f /mnt/drivefs/.file ]; then echo "mounted";else echo "!!! not mounted !!!";fi
echo -ne "\tPlexbot: "; if [ "$(ps aux |grep irssi |grep -v "grep" |awk '{print $11}')" = "irssi" ]; then echo "running"; else echo "not running";fi
